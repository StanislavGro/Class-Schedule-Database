...
vector<schedule*> DataMapper::fillAllVectors()
{
    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
        ...
        retcode = SQLBindCol(hstmt, 4, SQL_C_SLONG, &id_time, sizeof(id_time), NULL);
        retcode = SQLBindCol(hstmt, 5, SQL_C_SLONG, &id_group, sizeof(id_group), NULL);
        retcode = SQLBindCol(hstmt, 6, SQL_C_SLONG, &id_auditory, sizeof(id_auditory), NULL);
        ...
        for (int i = 1;; i++)
        {
            ...
            if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
                ...
                for (int i = 0, k = 0; i < 20; i++) {
                    ...
                    if (k == 2)
                        break;
                    newTimeStar.push_back(time_start[i]);
                }
